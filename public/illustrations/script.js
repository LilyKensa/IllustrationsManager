...
function loadImage(imageUrl, onProgress) {
    return new Promise((resolve, reject) => {
        ...
        xhr.addEventListener("loadend", (ev) => {
            ...
            resolve(window.URL.createObjectURL(new Blob([xhr.response], options)));
        });
        xhr.send();
    });
}
...
function start() {
    ...
    const data = shuffle(JSON.parse(document.querySelector(".data").textContent));
    function showFullscreenImage(ev) {
        let fullscreenContainer = document.querySelector(".fullscreen-container"), backdropEl = fullscreenContainer.querySelector(".backdrop"), progressEl = document.querySelector(".progress");
        ...
        function displayOldEl(show) {
            let oldEl = document.querySelector(`[data-compressed-img-src="${el.getAttribute("data-compressed-img-src")}"]`);
        ...
        function handleResize(cover = false) {
            ...
            if (cover)
                ...
                padding = Math.min(20, window.innerWidth * 0.025);
            let width, height;
            if (cover != (imgEl.naturalWidth / imgEl.naturalHeight <
        ...
        let mouse = {
            ...
            handleDrag() {
                if (mouse.dragging) {
                    ...
                    imgEl.style.transform = `translate(${-mouse.px * (width * mouse.scale - window.innerWidth)}px, ${-mouse.py * (height * mouse.scale - window.innerHeight)}px) scale(${mouse.scale})`;
                }
                else {
                    handleResize();
                    el.style.cursor = "";
        ...
        function handleHistoryBackButton(ev) {
            if (ev.button === 3) {
                ev.preventDefault();
                closeLargeImage();
            }
    ...
    for (let line of lines) {
        ...
        for (let img of line.content) {
            ...
            function checkIfInView() {
                const { top, left, bottom, right } = imgEl.getBoundingClientRect();
                if (((top > 0 && top < window.innerHeight) ||
                    (bottom > 0 && bottom < window.innerHeight)) && ((left > 0 && left < window.innerWidth) ||
                    (right > 0 && right < window.innerWidth)) && imgEl.complete)
